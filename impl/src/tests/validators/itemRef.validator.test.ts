...
>
  ...
  <any_element item_ref="unmapped_property"/>
    ...
    const result = await itemRefValidator(query);
    expect(result.map(e => e.message).join("\n")).toBe([
        "ValidationError: other_property at //any_element[0]@item_ref not in [rule_definition]",
        "ValidationError: unmapped_property at //any_element[1]@item_ref not in [rule_definition]"
      ].join("\n")
    )
  })
});