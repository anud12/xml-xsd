...
public class ClassifyPerson {
    ...
    public static Stream<String> getPersonClassifications(WorldStepInstance worldStepInstance, final Person person) {
        try (var scope = logScope("personId", person)){
            scope.logTodo("remove default empty classification");
            ...
            return scope.logReturn(Stream.concat(computedClassificationList, staticClassificationId.stream())
                    .peek(string -> {
