...
public class RegionInstance {
    ...
    public Region createRegion(Entry regionRule) {
        try (var scope = logScope("regionRule: ", regionRule.getId())) {
            ...
            return scope.logReturn(builder.build());
        }
    }
    ...
    private LimitRecord getLimits(Region region) {
        try (var scope = logScope("region:", region.getId())) {
            var height = switch (region.getPosition().getRotation()) {
                ...
                default -> {
                    scope.log("Unknown rotation: " + region.getPosition().getRotation());
            ...
            return scope.logReturn(new LimitRecord(height, width));
    ...
    public Position translateAwayBoundingBox(
        ...
        try (var scope = logScope("position:", position, "portal:", portal.getId())) {
            ...
            var returnValue = switch (region.getPosition().getRotation()) {
                case "normal" -> switch (portal.getSide()) {
                    ...
                    default -> {
                        scope.log("Unknown portal side: " + portal.getSide());
                ...
                default -> {
                    scope.log("Unknown position rotation: " + region.getPosition().getRotation());
                    throw new IllegalArgumentException("Unknown position rotation: " + region.getPosition().getRotation());
                }
            };
            return scope.logReturn(returnValue);
        }
    ...
    private Position translateDestinationPortal(
        ...
        try (var scope = logScope("position:", position, "fromRegion:", fromRegion.getId(), "fromPortal:", fromPortal.getId(), "targetRegion:", targetRegion.getId())) {
    ...
    public Position translateToBoundingBox(
        ...
        final Region region) {
        try (var scope = logScope("position:", position, "fromRegion:", fromRegion.getId(), "parentPortal:", parentPortal.getId(), "region:", region.getId())) {
    ...
    private Position rotateTargetPosition(
        ...
        try (var scope = logScope("position:", position, "availablePortal:", availablePortal, "toPortalRule:", toPortalRule)) {
            ...
            var returnValue = switch (fromRegion.getPosition().getRotation()) {
                ...
                default -> {
                    scope.log("Unknown position rotation: " + fromRegion.getPosition().getRotation());
    ...
    private void computePosition(final Portal parentPortal, final Region parentRegion, final Region targetRegion) {
        try (var scope = logScope("parentPortal:", parentPortal, "parentRegion:", parentRegion, "targetRegion:", targetRegion)) {
            ...
            scope.log("computed position", position.getX(), position.getY());
        }
