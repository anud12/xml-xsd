...
export const dependantTypeClearChild = (dependantType: DependantType): { dependantTypes:DependantType[], templateString: string } | undefined => {
    ...
    .map(([key, value]) => {
      ...
      return template()`
        ${value.isSingle && template()`
          if(linkedNode is ${type}) 
          {
            this.${normalizeName(key)} = ${value.isNullable ? "null" : "new()"}; 
          }
          
        `}
        ${!value.isSingle && template()`
          if(linkedNode is LinkedNodeCollection<${type}>) 
          {
            this.${normalizeName(key)} = ${value.isNullable ? "null" : "new()"};
          }
        `}
